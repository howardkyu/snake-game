...
void messageHandler(int clientID, string message){
    ...
    handlesMessage(clientID, message();)
...
int MoveHandler(int clientID, string dirString)
{
    ...
    if (dirString != "NULL")
    {
        playerMap[clientID]->direction = strToDir(dirString);
    }
    
    //create a outstream
    ostringstream os;
    //set outstream formated: "TYPE:FOOD POSX, FOOD POSY:P1 body:P2 body:P1 Score:P2 Score
    os << "GAMESTATE:" << game.foodPosition.x << "," << game.foodPosition.y << ":" << game.player1.bodyToString() << ":" << game.player2.bodyToString() << ":" << game.player1.score << ":" << game.player2.score;
